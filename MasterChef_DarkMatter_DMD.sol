...
contract DarkMatter is DelegateERC20, Ownable {
    uint256 private constant initialSupply = 935000 * 1e18;  
    uint256 private constant maxSupply = 100000 * 1e18;      
    ...
    function Total_DarkMatter_Burn() public view returns (uint256) {
        return _burnTotal;
    }
...
contract MasterChef_DarkMatter_DMD is Ownable, ReentrancyGuard {
    ...
    // Info of each user.
    struct UserInfo {
        uint256 amount;         // How many LP DarkMatters the user has provided.
        ...
        //
        // Whenever a user deposits or withdraws LP DarkMatters to a pool. Here's what happens:
    ...
    // Info of each pool.
    struct PoolInfo {
        IERC20 lpDarkMatter;           // Address of LP DarkMatter contract.
    ...
    // DMD DarkMatters created per block.
    ...
    // Info of each user that stakes LP DarkMatters.
    ...
    modifier nonDuplicated(IERC20 _lpDarkMatter) {
        require(poolExistence[_lpDarkMatter] == false, "nonDuplicated: duplicated");
        _;
    }

    // Add a new lp to the pool. Can only be called by the owner.
    function add(uint256 _allocPoint, IERC20 _lpDarkMatter, uint16 _depositFeeBP, bool _withUpdate) public onlyOwner nonDuplicated(_lpDarkMatter) {
        ...
        poolExistence[_lpDarkMatter] = true;
        poolInfo.push(PoolInfo({
        lpDarkMatter : _lpDarkMatter,
    ...
    function pendingDMD(uint256 _pid, address _user) external view returns (uint256) {
        ...
        uint256 lpSupply = pool.lpDarkMatter.balanceOf(address(this));
    ...
    // Deposit LP DarkMatters to MasterChef for DMD allocation.
    function deposit(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (user.amount > 0) {
          ...
          if (_amount > 0) {
            uint256 balanceBefore = pool.lpDarkMatter.balanceOf(address(this));
            pool.lpDarkMatter.safeTransferFrom(address(msg.sender), address(this), _amount);
            _amount = pool.lpDarkMatter.balanceOf(address(this)) - balanceBefore;
        
            if (pool.depositFeeBP > 0) {
                uint256 depositFee = _amount.mul(pool.depositFeeBP).div(10000);
                pool.lpDarkMatter.safeTransfer(feeAddress, depositFee);
    ...
    // Withdraw LP DarkMatters from MasterChef.
    function withdraw(uint256 _pid, uint256 _amount) public nonReentrant {
        ...
        if (_amount > 0) {
            ...
            pool.lpDarkMatter.safeTransfer(address(msg.sender), _amount);
        }
        user.rewardDebt = user.amount.mul(pool.accDMDPerShare).div(1e12);
        emit Withdraw(msg.sender, _pid, _amount);
    }
    
     // Stake  DMD DarkMatters to MasterChef
    ...
    // Withdraw DMD DarkMatters from STAKING.
    ...
    function emergencyWithdraw(uint256 _pid) public nonReentrant {
        ...
        user.amount = 0;
        user.rewardDebt = 0;
        pool.lpDarkMatter.safeTransfer(address(msg.sender), amount);
